...
namespace LuckyPicker
{
    ...
    public static class AppConfig
    {
        ...
        public static void Load()
        {
            try
            {
                ...
                var dict = ser.Deserialize<Dictionary<string, object>>(File.ReadAllText(ConfigPath));
                if (dict == null) return;
                object v;
                if (dict.TryGetValue("ttsOnline", out v) && v is bool) TtsOnline = (bool)v;
                if (dict.TryGetValue("ttsVoice", out v) && v is string && !string.IsNullOrEmpty((string)v)) TtsVoice = (string)v;
                if (dict.TryGetValue("ttsSource", out v) && v is string && !string.IsNullOrEmpty((string)v)) TtsSource = (string)v;
                if (dict.TryGetValue("edgeBlocked", out v) && v is bool) EdgeBlocked = (bool)v;
                if (dict.TryGetValue("updateUrl", out v) && v is string && !string.IsNullOrEmpty((string)v)) UpdateUrl = (string)v;
    ...
    public static class MsTts
    {
            ...
            lock (sync)
            {
                ...
                try
                {
                    ...
                    using (var sr = new StreamReader(s, Encoding.UTF8))
                    {
                        ...
                        string r = (d != null && d.ContainsKey("r")) ? Convert.ToString(d["r"]) : null;
                        string t = (d != null && d.ContainsKey("t")) ? Convert.ToString(d["t"]) : null;
        ...
        static long ParseJwtExp(string jwt)
        {
            try
            {
                ...
                var ser = new JavaScriptSerializer();
                var d = ser.Deserialize<Dictionary<string, object>>(json);
                if (d != null && d.ContainsKey("exp"))
                {
                    long e;
                    if (long.TryParse(Convert.ToString(d["exp"]), out e)) return e;
                }
            }
            catch { }
            return 0;
    ...
    public class TtsEngine : IDisposable
    {
        ...
        public TtsEngine(Control owner, Action<string> onStatus)
        {
            this.owner = owner;
